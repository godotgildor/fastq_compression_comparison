...
from tools.ubam import UbamCompressor


class UcramCompressor(BaseCompressor):
    """Unmapped CRAM via samtools import. Read order will differ from original."""
    ...
    def compress(self, fwd_reads, rev_reads, output_prefix, num_threads=1, **kwargs) -> CompressResult:
        output_file = Path(str(output_prefix) + ".cram")
        cmd = ["samtools", "import", "-@", str(num_threads), "-O", "cram", "-o", str(output_file)]
        if rev_reads:
    ...
    def get_version(self) -> str:
        return UbamCompressor().get_version()
